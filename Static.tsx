...
import { Constant, QuestionPreview, QuestionPreviewAdd, TestEdit } from 'store/selector';
import { updateQuestionPreview, updateQuestionPreviewAdd } from 'store/slice/question';
...
const CreateStaticTest = ({
  ...
  const questionPreviewAddRedux = useSelector(QuestionPreviewAdd);
  ...
  const [isOpenPreviewAdd, setIsOpenPreviewAdd] = useState<boolean>(false);
        ...
        if (res.status) {
          ...
          setPagiantion((pre: any) => {
            return {
              ...pre,
              total:
                Math.ceil(res.total / pagination.pageSize) > 0
                  ? Math.ceil(res.total / pagination.pageSize)
                  : 1,
            };
          });
          setIsLoading(false);
  ...
  const handlePreviewQuestion = (id: number) => {
    const index = listQuestionSelected.findIndex((item: any) => item.idQuestion === id);
    dispatch(updateQuestionPreview(listQuestionSelected[index]));
    setIsOpenPreview(true);
  };

  const handlePreviewQuestionAdd = (id: number) => {
    const index = listQuestion.findIndex((item: any) => item.idQuestion === id);
    dispatch(updateQuestionPreviewAdd(listQuestion[index]));
    setIsOpenPreviewAdd(true);
  };
  ...
  useEffect(() => {
    setPagiantion((pre: any) => {
      return {
        ...pre,
        total: Math.ceil(pagination.pageSize / pagination.pageSize),
      };
    });
    setIsLoading(true);
    RequestAPI({
  ...
  useEffect(() => {
    if (testRedux?.listQuestion?.length > 0) {
      ...
      localStorage.setItem('selectedQuestion', JSON.stringify(testRedux.listQuestion));
      setListQuestion(testRedux.listQuestion);
      const clone = JSON.parse(JSON.stringify(testRedux.listQuestion));
      clone.forEach((i: any) => {
        i['checked'] = true;
      });
      setListQuestionTemp(clone);
                ...
                <ScrollArea style={{ height: '300px', width: '96%' }}>
                  {isOpenPreviewAdd ? (
                    <div className='px-2'>
                      <div
                        className='flex items-center bg-ct-secondary w-fit rounded-md text-white cursor-pointer p-2 mb-4'
                        onClick={() => setIsOpenPreviewAdd(false)}
                      >
                        <ArrowLeft size='25' color='currentColor' />
                        <p className='px-2'>Quay lại</p>
                      </div>
                      <div>
                        <PreviewQuestion
                          quiz_type={questionPreviewAddRedux?.quiz_type}
                          data={questionPreviewAddRedux}
                        />
                      </div>
                    </div>
                  ) : (
                              ...
                              preview: (
                                <>
                                  <Eye
                                    ...
                                    onClick={() => {
                                      handlePreviewQuestionAdd(item.idQuestion);
                                    }}
                                  />
                                </>
                              ),
