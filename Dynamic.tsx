...
const CreateDynamicTest = ({
  ...
  useEffect(() => {
    ...
    listCondition.map((item: any) => {
      ...
      if (item.quiz_type === questionEnumType.READING) {
              ...
              <div className='w-1/2 flex'>
                ...
                <div className='w-1/2'>
                  ...
                  <p className='pr-4 pb-3'>
                    {item?.listTag
                      .map((item: any) => {
                        return tagList.filter((item2: any) => +item2?.idTag === +item)[0]?.name;
                      })
                      .join(', ') || 'Tất cả'}
                  </p>
                  <p className='pr-4 pb-3'>
                    {item?.awareness_level
                      ? constantForm.question.awareness_level[Number(item?.awareness_level)]?.title
                      : 'Tất cả'}
                  </p>
                  <p className='pr-4 pb-3'>
