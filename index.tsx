...
const CreateTestContainer = () => {
  ...
  const [listExamFormat, setlistExamFormat] = useState<any>([]);
  ...
  const handleCompleteStep = () => {
    ...
    if (isPassQuestionForm && step === 1) {
      ...
      if (isPassConditionForm && !error && same) {
        ...
        const tag: number[] = [];
        questionForm?.values.listTag.map((item1: any) => {
        ...
        const payload = {
          baikiemtra: {
            ...
            listClass,
            listSubject,
            awareness_level: questionForm.values.awareness_level,
            ...
            examFormat: questionForm.values.examFormat,
          },
          test: {
            name: questionForm.values.name,
            ...
            RequestAPI({
              ...
              .then((res: any) => {
                if (res.status) {
                  ...
                  AddToCourse(testRedux.idBaikiemtra);
                }
              })
              .catch((err: any) => {
                setSuccessful(false);
  ...
  const AddToCourse = async (id: any) => {
    ...
    if (data !== null) {
      ...
      if (testRedux.idBaikiemtra == undefined || testRedux.idBaikiemtra == '') {
        RequestAPI({
      ...
      return navigate(
        `/manage-course/${idCourse}/#id${
          parseData.idSection !== null ? parseData.idSection : parseData.idLesson
        }`
      );
    }
  };
  ...
  useEffect(() => {
    const listExam = constantForm?.baikiemtra?.examFormat.filter(
      (item: any) => item.title != 'Bài làm thử cuộc thi'
    );
    setlistExamFormat(listExam);
    ...
    if (testRedux?.idTest) {
      RequestAPI({
        ...
        if (res.status) {
          ...
          questionForm.setFieldValue('examFormat', Number(data.examFormat));
          questionForm.setFieldValue('awareness_level', Number(data.awareness_level));
    ...
    if (testRedux?.listQuestion?.length > 0) {
      ...
      questionForm.setFieldValue('listTag', reformatTagList);
    ...
    <div className='w-full'>
      ...
      <div className='w-full'>
        ...
        {step == 0 && constantForm.question && (
          ...
          >
            ...
            <Select
              ...
              data={constantForm?.question?.awareness_level?.map((item: any) => {
                const awarenessLevelTemp = {
                  value: '',
                  label: '',
                };
                awarenessLevelTemp['label'] = item.title;
                awarenessLevelTemp['value'] = '' + item.value;
                return awarenessLevelTemp;
              })}
              onChange={(value: any) => {
                questionForm.setFieldValue('awareness_level', Number(value));
              }}
              value={'' + questionForm.values.awareness_level}
            />
            <MultiSelect
            ...
            <Select
              {...questionForm.getInputProps('examFormat')}
              className='w-1/3 px-7 py-3'
              radius={15}
              rightSection={<ArrowDown2 size={15} color='currentColor' variant='Bold' />}
              styles={{ rightSection: { pointerEvents: 'none' } }}
              ...
              data={listExamFormat.map((item: any) => {
                item['label'] = item.title;
                item['value'] = item.value.toString();
                item['key'] = item.value;
              ...
              value={'' + questionForm.values.examFormat}
              onChange={(value: any) => {
                questionForm.setFieldValue('examFormat', Number(value));
              }}
            />
                      ...
                      label={
                        <>
                          <Tooltip
                            ...
                            transition='fade'
                            transitionDuration={200}
                            label='Nếu học viên chọn sai đáp án sẽ bị trừ điểm'
                          >
                            Sai bị trừ điểm
                            <InfoCircle
                              size='20'
        ...
        {step === 2 && (
          <div>
            {!successful ? (
              <p className='text-center text-ct-red-500 font-bold text-2xl my-12'>
                Tạo đề không thành công!
              </p>
            ) : (
        ...
        <div className='flex justify-end items-center w-full mt-10'>
          {step >= 1 && (
            <Button
              ...
              onClick={() => {
                if (step === 2) {
                  return (window.location.href = '/manage-topic/create');
                } else {
                  setStep(step - 1);
                }
              }}
